...
#ifdef PUFFER_BALATRO_PARALLEL_SAMPLER
#include "../ocean/balatro/policy.h"
#endif
...
//   Balatro autoregressive condition table, when present
...
#ifdef PUFFER_BALATRO_PARALLEL_SAMPLER
    net->ar_condition = get_weights_aligned(weights, BALATRO_AR_CONDITION_SIZE);
#endif
    if (!net->is_continuous) {
        net->multidiscrete = make_multidiscrete(num_agents, logit_sizes, num_actions);
    }
    ...
    while (!WindowShouldClose()) {
        if (frame % 4 == 0) {
            for (int i = 0; i < env.num_agents * OBS_SIZE; ++i)
                net->obs[i] = (float)observations[i] * PUFFER_BYTE_OBS_SCALE;
