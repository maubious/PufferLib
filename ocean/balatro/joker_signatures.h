...
/* Typed joker effect table, transcribed from the game logic (scoring.c,
   economy.c, step.c, balatro.c, shop.c, packs.c). Consumers attach each live
   joker's signature to its observation slot so the policy can evaluate
   purchases without inferring effects from reward correlation.

   Semantics:
   - trigger: when the effect fires.
   - condition / cond_value / rank_mask: which cards it applies to.
     rank_mask: bit (rank-2) per rank (2..14, 14=A).
   - chips / mult / dollars: additive magnitude per trigger event.
   - xmult_pct: multiplicative mult in percent. 100 = x1.0, 150 = x1.5.
     With scale != NONE the total is linear in units:
       1 + (xmult_pct - 100) / 100 * units
     (Steel Joker 120: x(1 + 0.2*steels); Stencil 200: x(free_slots + 1)).
   - scale: magnitude is per unit of a state-derived quantity (Bull: +2
     chips per dollar). Applies to the listed channel(s).
   - growth != 0: stateful joker; magnitude lives in the slot's state[0]
     (already in the observation). Listed magnitude is the starting value,
     growth is the change per trigger (per round for ROUND_END).
   - chance_pct < 100: probabilistic trigger.
   - retriggers: extra scoring repetitions of each matching card.
   - levels: hand levels gained per trigger.
   - hand_size / discards / hands: logistic deltas while owned.
   - sell_growth: sell value gained per round.
   - rule: hand-classification modifier.
   - opaque: effect depends on the rest of the build (copy machinery,
     destruction chains) or resists static typing.
   - nondeterministic: effect is random; expected magnitude listed.
*/
...
/* Entry shorthand: unspecified fields are zero (= NONE / 100% / x1.0). */
#define SIG_ENTRY(...) { __VA_ARGS__ }

static const JokerSignature JOKER_SIGNATURES[CENTER_COUNT] = {
    ...
    [CENTER_J_SPACE]           = {.trigger=SIG_TRIG_HAND_PLAYED, .levels=1, .chance_pct=25, .nondeterministic=1},
    ...
    [CENTER_J_TURTLE_BEAN]     = {.hand_size=5, .growth=-1},
