...
static const uint8_t hand_order[] = {
...
static inline int can_add_playing_cards(const State *state, uint16_t count) {
    return state->deck_count + state->hand_count + state->discard_count + count <= MAX_DECK;
}
...
// -----------------------
// --- Hand and deck mechanics ----

static uint8_t visible_hands(const State *state, int excluded, uint8_t out[HAND_COUNT]) {
    uint8_t count = 0;
    for (size_t i = 0; i < sizeof(hand_order); ++i) {
        uint8_t hand = hand_order[i];
        if ((hand >= STRAIGHT_FLUSH || state->hand_plays[hand] != 0) && (int)hand != excluded)
            out[count++] = hand;
    }
    return count;
}

void sort_hand(State *state, int by_suit) {
    int keys[MAX_HAND];
...
static void fill_shop_jokers(State *state) {
    for (uint8_t i = 0; i < state->shop_joker_max; ++i) {
        ...
        if (state->shop_main_count < MAX_SHOP_MAIN)
...
void populate_shop(State *state) {
    ...
    if (state->next_voucher_id && state->shop_voucher_count < MAX_SHOP_VOUCHERS)
        state->shop_vouchers[state->shop_voucher_count++] = center_card(state, state->next_voucher_id);
    while (state->tag_voucher_pending && state->shop_voucher_count < MAX_SHOP_VOUCHERS) {
        Card free_voucher = center_card(state, pick_voucher_stream(state, "Voucher_fromtag"));
        free_voucher.cost = 0;
...
static void apply_voucher(State *state, uint16_t id) {
    ...
    case VOUCHER_SHOP_SIZE: {
        ...
        while (state->shop_main_count < state->shop_joker_max &&
               state->shop_main_count < MAX_SHOP_MAIN) {
            Card card = create_shop_card(state);
            state->shop_main[state->shop_main_count++] = card;
        }
        break;
    ...
    if (!hook && first_discard) {
        for (uint8_t j = 0; j < state->joker_count; ++j)
            if (!(state->jokers[j].flags & CARD_DEBUFFED) && state->jokers[j].center_id == CENTER_J_BURNT) {
                uint8_t mask = 0;
                HandType hand = classify_hand(cards, count, &mask, 0, 0, 0);
...
static void individual_jokers(State *state, const ScoreContext *context, const Card *played, size_t card_index,
    ...
    for (size_t source_index = 0; source_index < context->source_count; ++source_index) {
        ...
        if (sig->trigger == SIG_TRIG_SCORED_CARD && sig->growth == 0 &&
            ...
            case SIG_COND_SUIT: trigger = is_suit_state(state, card, sig->cond_value); break;
            case SIG_COND_RANK: trigger = card->rank == sig->cond_value; break;
            case SIG_COND_RANK_SET:
                trigger = (card->rank >= 2 && card->rank <= 14 && (sig->rank_mask & (1u << (card->rank - 2))));
                break;
            case SIG_COND_FACE: trigger = is_face(card, pareidolia); break;
            default: break;
            }
            if (trigger) {
                *chips += sig->chips;
                *mult += sig->mult;
                *dollars += sig->dollars;
                if (sig->xmult_pct) *mult *= (double)sig->xmult_pct / 100.0;
            }
        }
        switch (joker->center_id) {
        case CENTER_J_BLOODSTONE:
            if (is_suit_state(state, card, HEARTS) && pseudorandom(state, "bloodstone") < adjust_probability(state, 0.5))
                *mult *= 1.5;
            break;
        case CENTER_J_IDOL:
        ...
        case CENTER_J_BUSINESS:
            if (is_face(card, pareidolia) && pseudorandom(state, "business") < adjust_probability(state, 0.5)) *dollars += 2;
            break;
        case CENTER_J_HIKER:
        ...
        case CENTER_J_8_BALL:
            if (card->rank == 8 && state->consumable_count < state->consumable_slots &&
                pseudorandom(state, "8ball") < adjust_probability(state, 0.25)) {
...
static void end_round_jokers(State *state) {
    for (uint8_t i = 0; i < state->joker_count; ++i) {
        ...
        if (joker->center_id == CENTER_J_POPCORN) {
            ...
            if (pseudorandom(state, stream) < adjust_probability(state, 1.0 / odds)) {
        ...
        for (int repetition = 0; repetition < repetitions; ++repetition) {
            ...
            for (size_t j = 0; j < context->source_count; ++j) {
                ...
                if (joker->center_id == CENTER_J_RESERVED_PARKING && is_face(&held[i], context->held_pareidolia) &&
                    pseudorandom(state, "parking") < adjust_probability(state, 0.5))
                    (*dollars)++;
            }
        }
    }
...
static void main_jokers(State *state, const ScoreContext *context, HandType hand, const Card *played,
    ...
    for (uint8_t j = 0; j < state->joker_count; ++j) {
        ...
        if (bypass) {
            if (physical->edition == EDITION_FOIL)
                *chips += 50;
            else if (physical->edition == EDITION_HOLO)
                *mult += 10;
            joker_on_joker_effects(state, j, mult);
            if (physical->edition == EDITION_POLYCHROME) *mult *= 1.5;
            continue;
        }
        ...
        if (sig->trigger == SIG_TRIG_HAND_PLAYED && sig->growth == 0 &&
            ...
            switch (sig->condition) {
            case SIG_COND_NONE: trigger = 1; break;
            case SIG_COND_MAX3: trigger = played_count <= 3; break;
            case SIG_COND_LAST_HAND: trigger = state->hands_left == 0; break;
            case SIG_COND_REPEAT_HAND: trigger = state->hand_plays_round[hand] > 1; break;
            case SIG_COND_NO_DISCARDS: trigger = state->discards_left == 0; break;
            case SIG_COND_HAND_TYPE: trigger = context->hand_has[sig->cond_value]; break;
            default: break;
            }
            if (sig->scale == SIG_SCALE_DECK_LOST) {
                int starting = state->config.deck == CENTER_B_ABANDONED ? 40 : 52;
                units = starting - context->playing_count;
                trigger = units > 0;
            } else if (trigger) {
                switch (sig->scale) {
                case SIG_SCALE_JOKERS: units = state->joker_count; break;
                case SIG_SCALE_DISCARD_LEFT: units = state->discards_left; break;
                case SIG_SCALE_DECK: units = state->deck_count; break;
                case SIG_SCALE_DOLLARS: units = state->dollars > 0 ? state->dollars : 0; trigger = state->dollars > 0; break;
                case SIG_SCALE_DOLLARS_5: units = state->dollars > 0 ? state->dollars / 5 : 0; trigger = state->dollars > 0; break;
                case SIG_SCALE_HAND_PLAYS: units = state->hand_plays[hand]; break;
                case SIG_SCALE_TAROTS: units = state->tarots_used; break;
                case SIG_SCALE_STONES: units = context->stones; break;
                case SIG_SCALE_STEELS: units = context->steels; break;
                case SIG_SCALE_SKIPS: units = state->skips; break;
                default: break;
                }
            }
            if (trigger && units > 0) {
                *chips += sig->chips * units;
                *mult += sig->mult * units;
                if (sig->xmult_pct) {
                    if (sig->condition == SIG_COND_HAND_TYPE || sig->scale == SIG_SCALE_NONE) {
                        *mult *= (double)sig->xmult_pct / 100.0;
                    } else {
                        *mult *= 1.0 + (double)(sig->xmult_pct - 100) / 100.0 * units;
                    }
                }
            }
        }
        if (joker->state[0] > 100) {
            ...
            case CENTER_J_VAMPIRE:
                *mult *= (double)joker->state[0] / 100.0;
                break;
            default: break;
            }
        }
        switch (joker->center_id) {
        case CENTER_J_DRIVERS_LICENSE:
            if (context->enhanced >= 16) *mult *= 3;
            break;
...
static int score_hand(State *state, const Card *played, size_t played_count, const Card *held, size_t held_count,
    ...
    for (size_t i = 0; i < played_count; ++i) {
        ...
        for (int repetition = 0; repetition < repetitions; ++repetition) {
            ...
            if (card->enhancement == ENHANCEMENT_LUCKY) {
                int lucky_trigger = 0;
                if (pseudorandom(state, "lucky_mult") < adjust_probability(state, 0.2)) {
                    mult += 20;
                    lucky_trigger = 1;
                }
                if (pseudorandom(state, "lucky_money") < adjust_probability(state, 1.0 / 15.0)) {
                    dollars += 20;
                    lucky_trigger = 1;
                }
                if (lucky_trigger)
    ...
    for (size_t i = 0; i < played_count; ++i) {
        if ((scoring & (1u << i)) && !(played[i].flags & CARD_DEBUFFED) && played[i].enhancement == ENHANCEMENT_GLASS &&
            pseudorandom(state, "glass") < adjust_probability(state, 0.25))
...
#define ADD(type_value, primary_value) \
    ...
    if (state->phase == PHASE_BLIND_SELECT) {
        ...
        for (uint8_t i = 0; i < state->shop_main_count && i < MAX_SHOP_MAIN; ++i) {
        ...
        for (uint8_t i = 0; i < state->shop_voucher_count && i < MAX_SHOP_VOUCHERS; ++i)
            if (can_afford(state, state->shop_vouchers[i].cost)) ADD(ACTION_REDEEM_VOUCHER, i);
        for (uint8_t i = 0; i < state->shop_booster_count && i < MAX_SHOP_BOOSTERS; ++i)
...
int state_layout_valid(const State *state) {
    return state && state->rng_count <= MAX_RNG_STREAMS && state->deck_count <= MAX_DECK &&
           ...
           state->consumable_count <= MAX_CONSUMABLES &&
           state->shop_main_count <= MAX_SHOP_MAIN &&
           state->shop_voucher_count <= MAX_SHOP_VOUCHERS &&
           state->shop_booster_count <= MAX_SHOP_BOOSTERS &&
           state->deck_count + state->hand_count + state->discard_count <= MAX_DECK &&
    ...
    *out = (Observation){0};
    ...
    for (int zone = 0; zone < 3; ++zone) {
        for (uint16_t i = 0; i < deck_counts[zone]; ++i) {
            ...
            assert(out->counts.special_count < MAX_DECK);
...
static void play_or_discard(State *state, const Action *action) {
    ...
    if (action->type == ACTION_PLAY_HAND) {
        ...
        if (!state->blind_disabled && state->blind_id == BLIND_BL_OX) {
            uint8_t mask = 0;
            HandType candidate = classify_hand(cards, (size_t)count, &mask, 0, 0, 0);
