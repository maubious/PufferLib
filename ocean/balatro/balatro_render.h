...
static void get_center_description(const Card *card, const State *state, char *out, size_t max_len) {
    ...
    switch (cid) {
        ...
        case CENTER_J_STENCIL: snprintf(out, max_len, "x1 Mult for each empty Joker slot"); break;
        ...
        case CENTER_J_LOYALTY_CARD: snprintf(out, max_len, "x4 Mult every 6 hands played (%d/6)", st); break;
        ...
        case CENTER_J_STEEL_JOKER: snprintf(out, max_len, "Gives x0.2 Mult for each Steel Card in full deck"); break;
        ...
        case CENTER_J_ODD_TODD: snprintf(out, max_len, "+30 Chips per scored odd card (A, 9, 7, 5, 3)"); break;
        ...
        case CENTER_J_BLUE_JOKER: snprintf(out, max_len, "+2 Chips for each remaining card in deck"); break;
        ...
        case CENTER_J_CONSTELLATION: snprintf(out, max_len, "Gains x0.1 Mult per Planet card used (Currently x%.1f)", 1.0f + st * 0.1f); break;
        case CENTER_J_HIKER: snprintf(out, max_len, "+4 Chips permanently to each played scoring card"); break;
        ...
        case CENTER_J_TODO_LIST: snprintf(out, max_len, "Earn $5 if poker hand matches target"); break;
        ...
        case CENTER_J_MADNESS: snprintf(out, max_len, "Gains x0.5 Mult when Small/Big Blind selected, drains random Joker"); break;
        ...
        case CENTER_J_VAMPIRE: snprintf(out, max_len, "Gains x0.2 Mult per Enhanced card played, removes enhancement"); break;
        case CENTER_J_SHORTCUT: snprintf(out, max_len, "Allows Straights to be made with gaps of 1 rank"); break;
        case CENTER_J_HOLOGRAM: snprintf(out, max_len, "Gains x0.25 Mult whenever a playing card is added to deck"); break;
        ...
        case CENTER_J_CLOUD_9: snprintf(out, max_len, "Earn $1 for each 9 in full deck at end of round"); break;
        case CENTER_J_ROCKET: snprintf(out, max_len, "Earn $1 at end of round, +$2 when Boss Blind defeated (Currently $%d)", st ? st : 1); break;
        case CENTER_J_OBELISK: snprintf(out, max_len, "x0.2 Mult per consecutive hand without playing most played poker hand"); break;
        ...
        case CENTER_J_EROSION: snprintf(out, max_len, "+4 Mult for each card below 52 in full deck"); break;
        case CENTER_J_RESERVED_PARKING: snprintf(out, max_len, "1 in 2 chance for face cards in hand to give $1"); break;
        case CENTER_J_MAIL: snprintf(out, max_len, "Earn $5 per discarded target rank"); break;
        ...
        case CENTER_J_FORTUNE_TELLER: snprintf(out, max_len, "+1 Mult per Tarot card used this run (Currently +%d)", st); break;
        ...
        case CENTER_J_STONE: snprintf(out, max_len, "+25 Chips for each Stone Card in deck"); break;
        case CENTER_J_GOLDEN: snprintf(out, max_len, "Earn $4 at end of round"); break;
        case CENTER_J_LUCKY_CAT: snprintf(out, max_len, "Gains x0.2 Mult each time a Lucky Card successfully triggers"); break;
        ...
        case CENTER_J_ANCIENT: snprintf(out, max_len, "x1.5 Mult per card scored of shifting suit"); break;
        case CENTER_J_RAMEN: snprintf(out, max_len, "x2 Mult, loses x0.01 Mult per card discarded"); break;
        ...
        case CENTER_J_CASTLE: snprintf(out, max_len, "Gains +3 Chips per discarded card of shifting suit (Currently +%d)", st); break;
        case CENTER_J_SMILEY: snprintf(out, max_len, "+5 Mult per scored Face card"); break;
        case CENTER_J_CAMPFIRE: snprintf(out, max_len, "Gains x0.25 Mult per card sold, resets on Boss defeated"); break;
        ...
        case CENTER_J_HIT_THE_ROAD: snprintf(out, max_len, "Gains x0.5 Mult per Jack discarded this round"); break;
        case CENTER_J_IDOL: snprintf(out, max_len, "Each scored target rank and suit gives x2 Mult; target changes each round"); break;
        ...
        case CENTER_J_TRIBE: snprintf(out, max_len, "x3 Mult if played hand contains a Flush"); break;
        ...
        case CENTER_J_SATELLITE: snprintf(out, max_len, "Earn $1 at end of round per unique Planet card used"); break;
        ...
        case CENTER_J_CAINO: snprintf(out, max_len, "Gains x1 Mult whenever a Face card is destroyed (Currently x%.1f)", 1.0f + st); break;
        case CENTER_J_TRIBOULET: snprintf(out, max_len, "x2 Mult per scored King and Queen"); break;
        case CENTER_J_YORICK: snprintf(out, max_len, "x5 Mult after discarding 23 cards (Discarded: %d/23)", st); break;
        ...
        case CENTER_J_BLOODSTONE: snprintf(out, max_len, "1 in 3 chance for scored Hearts to give x2 Mult"); break;
        case CENTER_J_ARROWHEAD: snprintf(out, max_len, "+50 Chips per scored Spade"); break;
        case CENTER_J_ONYX_AGATE: snprintf(out, max_len, "+8 Mult per scored Club"); break;
        ...
        case CENTER_J_SWASHBUCKLER: snprintf(out, max_len, "Adds sell value of all other Jokers to Mult (Currently +%d)", st); break;
        case CENTER_J_THROWBACK: snprintf(out, max_len, "x0.25 Mult per Blind skipped this run (Currently x%.2f)", 1.0f + st * 0.25f); break;
        ...
        case CENTER_J_GLASS: snprintf(out, max_len, "Gains x0.75 Mult for each Glass Card destroyed"); break;
...
static void draw_playing_card(const Card *card, Rectangle r, bool selected, bool hovered, bool in_shop, int cost) {
    if (selected) {
        r.y -= 22.0f;
    } else if (hovered) {
        r.y -= 8.0f;
...
static void draw_joker_card(const Card *card, Rectangle r, bool hovered, int index, bool in_shop, int cost, Texture2D puffer) {
    if (hovered) {
        r.y -= 6.0f;
    }
    ...
    Color bg_color = (Color){240, 242, 245, 255};
    if (card->edition == EDITION_NEGATIVE) {
        bg_color = (Color){18, 20, 26, 255};
    }

    /* Card background */
    DrawRectangleRounded(r, 0.08f, 4, bg_color);

    /* Rarity frame */
    DrawRectangleRoundedLinesEx(r, 0.08f, 4, hovered ? 3.0f : 2.0f, hovered ? BALATRO_GOLD : rarity_color);

    /* Header Pill */
    DrawRectangleRounded((Rectangle){r.x + 3, r.y + 3, r.width - 6, 16}, 0.2f, 2, rarity_color);
    DrawText(rarity_name, (int)(r.x + r.width * 0.5f - (MeasureText(rarity_name, 10) / 2)), (int)(r.y + 5), 10, BALATRO_WHITE);

    /* Joker Name with Multi-line Wrapping */
    ...
    /* PufferLib mascot portrait; title and value bands remain unobstructed. */
    float cx = r.x + r.width * 0.5f;
    float cy = r.y + r.height * 0.54f;
    float portrait_radius = r.width < 75.0f ? 14.0f : 17.0f;
    DrawCircleV((Vector2){cx, cy}, portrait_radius, Fade(rarity_color, 0.16f));
    DrawCircleLines((int)cx, (int)cy, portrait_radius, rarity_color);
    assert(puffer.id > 0);
    ...
    if (card->state[0] > 0 || card->perma_bonus > 0) {
        char stat_buf[32];
        if (card->perma_bonus > 0) {
            snprintf(stat_buf, sizeof(stat_buf), "+%d Chips", card->perma_bonus);
        } else {
            snprintf(stat_buf, sizeof(stat_buf), "+%d", (int)card->state[0]);
        }
        DrawRectangleRounded((Rectangle){r.x + 6, r.y + r.height - 22, r.width - 12, 16}, 0.3f, 2, BALATRO_HEADER_BG);
        DrawText(stat_buf, (int)(r.x + r.width * 0.5f - (MeasureText(stat_buf, 10) / 2)), (int)(r.y + r.height - 19), 10, BALATRO_GOLD);
    } else {
        /* Sell value */
        char sell_buf[16];
        snprintf(sell_buf, sizeof(sell_buf), "$%d", in_shop ? cost : card->sell_cost);
        DrawRectangleRounded((Rectangle){r.x + 10, r.y + r.height - 20, r.width - 20, 15}, 0.3f, 2, in_shop ? BALATRO_GOLD : (Color){70, 75, 80, 255});
        DrawText(sell_buf, (int)(r.x + r.width * 0.5f - (MeasureText(sell_buf, 10) / 2)), (int)(r.y + r.height - 18), 10, in_shop ? BALATRO_DARK_TEXT : BALATRO_WHITE);
    }
}
...
static void draw_consumable_card(const Card *card, Rectangle r, bool hovered, int index, bool in_shop, int cost) {
    ...
    DrawRectangleRoundedLinesEx(r, 0.08f, 4, hovered ? 2.5f : 1.5f, hovered ? BALATRO_GOLD : BALATRO_WHITE);

    /* Set Header */
    DrawRectangleRounded((Rectangle){r.x + 4, r.y + 4, r.width - 8, 14}, 0.2f, 2, Fade(BLACK, 0.35f));
    DrawText(set_name, (int)(r.x + r.width * 0.5f - (MeasureText(set_name, 9) / 2)), (int)(r.y + 6), 9, BALATRO_WHITE);

    /* Name with Multi-line Wrapping */
    const char *name = get_center_name(card->center_id);
    int font_size = 9;
    if (r.width < 55.0f) font_size = 8;
    ...
    if (def->set == SET_PLANET) {
        ...
        DrawCircleGradient((int)cx, (int)cy, 14, (Color){120, 255, 255, 255}, Fade(body_color, 0.0f));
    }
...
static void draw_hud_sidebar(const State *state, const LegalMasks *legal, Rectangle rect, int selected_count, const uint8_t *selected_indices, Action *out_action, Vector2 mouse, Client *client) {
    ...
    /* Hands Left & Discards Left boxes */
    float box_w = (w - 10) * 0.5f;
    /* Hands Left */
    DrawRectangleRounded((Rectangle){x, y, box_w, 54}, 0.2f, 2, BALATRO_HEADER_BG);
    DrawText("HANDS", (int)(x + 10), (int)(y + 6), 11, (Color){180, 200, 220, 255});
    DrawText(TextFormat("%d", state->hands_left), (int)(x + 10), (int)(y + 20), 26, BALATRO_BLUE_CHIPS);

    /* Discards Left */
    DrawRectangleRounded((Rectangle){x + box_w + 10, y, box_w, 54}, 0.2f, 2, BALATRO_HEADER_BG);
    DrawText("DISCARDS", (int)(x + box_w + 20), (int)(y + 6), 11, (Color){220, 180, 180, 255});
    DrawText(TextFormat("%d", state->discards_left), (int)(x + box_w + 20), (int)(y + 20), 26, BALATRO_RED_MULT);
    ...
    DrawText(TextFormat("%d / %d", state->deck_count, state->deck_count + state->discard_count + state->hand_count), (int)(x + box_w + 20), (int)(y + 20), 16, BALATRO_WHITE);

    y += 56.0f;

    /* Hand Evaluator Box (When playing hands) */
    if (state->phase == PHASE_SELECTING_HAND || state->phase == PHASE_ROUND_EVAL) {
        ...
        if (selected_count > 0) {
            /* Classify played cards */
            Card played[MAX_SELECTION];
            for (int i = 0; i < selected_count; ++i) {
                played[i] = state->hand[selected_indices[i]];
            }
            uint8_t scoring_mask = 0;
            HandType evaluated = classify_hand(played, (size_t)selected_count, &scoring_mask, 0, 0, 0);
            int level = state->hand_levels[evaluated] ? state->hand_levels[evaluated] : 1;
            int base_chips = 0, base_mult = 0;
            hand_base_stats(evaluated, level, &base_chips, &base_mult);

            DrawText(get_hand_type_name(evaluated), (int)(x + 10), (int)(y + 8), 16, BALATRO_GOLD);
            ...
            const char *chips_text = TextFormat("%d Chips", base_chips);
            ...
            const char *mult_text = TextFormat("%d Mult", base_mult);
            DrawText(mult_text, (int)(mult_box.x + (mult_box.width - MeasureText(mult_text, 14)) * 0.5f),
                     (int)(mult_box.y + 7), 14, BALATRO_WHITE);

            DrawText(TextFormat("Base Score: %d", base_chips * base_mult), (int)(x + 12), (int)(y + 74), 13, (Color){180, 220, 255, 255});
        } else {
            DrawText("NO CARDS SELECTED", (int)(x + 12), (int)(y + 16), 13, (Color){150, 160, 170, 255});
            DrawText("Select 1 to 5 cards from", (int)(x + 12), (int)(y + 42), 11, (Color){120, 130, 140, 255});
            DrawText("your hand to evaluate", (int)(x + 12), (int)(y + 58), 11, (Color){120, 130, 140, 255});
        }
        y += 120.0f;
    }
...
static void draw_jokers_bar(const State *state, Rectangle rect, Action *out_action, Vector2 mouse, Client *client) {
    ...
    for (int i = 0; i < state->joker_count; ++i) {
        ...
        draw_joker_card(&state->jokers[i], card_r, hovered, i, false, 0, client->puffer);

        if (hovered) {
            client->show_tooltip = true;
            snprintf(client->tooltip_title, sizeof(client->tooltip_title), "%s", get_center_name(state->jokers[i].center_id));
            snprintf(client->tooltip_type, sizeof(client->tooltip_type), "Joker | Left/Right: Move | Right-click: Sell");
            get_center_description(&state->jokers[i], state, client->tooltip_desc, sizeof(client->tooltip_desc));
            snprintf(client->tooltip_stats, sizeof(client->tooltip_stats), "Slot %d of %d", i + 1, state->joker_count);
            client->tooltip_pos = (Vector2){mouse.x + 16, mouse.y + 16};
...
static void draw_phase_hand_play(State *state, const LegalMasks *legal, Rectangle rect, int *selected_count, uint8_t *selected_indices, Action *out_action, Vector2 mouse, Client *client) {
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            draw_playing_card(&state->hand[i], card_r, is_selected, hovered, false, 0);
...
static void score_anim_capture(Client *client, const State *state, const Action *action) {
    ...
    for (int i = 0; i < action->selection_count && i < MAX_SELECTION; ++i)
        a->played[i] = state->hand[action->selection[i]];
    uint8_t mask = 0;
    a->hand_type = classify_hand(a->played, (size_t)a->played_count, &mask, 0, 0, 0);
    a->level = state->hand_levels[a->hand_type] ? state->hand_levels[a->hand_type] : 1;
    hand_base_stats(a->hand_type, a->level, &a->base_chips, &a->base_mult);
...
static void draw_phase_scoring(const Client *client, Rectangle rect) {
    ...
    /* Played cards: rise + settle into a fan */
    float card_w = 64.0f, card_h = 92.0f;
    float pop = ease_out_back((a->t < 0.24f ? a->t : 0.24f) / 0.24f);
    float card_scale = 0.5f + 0.5f * pop;
    float card_y = cy - card_h * 0.5f - 34.0f + (1.0f - pop) * 90.0f;
    float pitch = (a->played_count > 1) ? 34.0f : 0.0f;
    float total_w = (a->played_count - 1) * pitch;
    for (int i = 0; i < a->played_count; ++i) {
        float x = cx - total_w * 0.5f + i * pitch;
        float w = card_w * card_scale, h = card_h * card_scale;
        draw_playing_card(&a->played[i], (Rectangle){x - w * 0.5f, card_y - (h - card_h) * 0.5f, w, h}, false, false, false, 0);
    }
    ...
    if (a->t > 0.38f) {
        ...
        DrawText(TextFormat("%d", (int)(a->base_chips * ct)), (int)(x0 + 12), (int)(y0 + 8), 18, BALATRO_WHITE);
        ...
        DrawText(TextFormat("%d", (int)(a->base_mult * ct)), (int)(x1 + 12), (int)(y0 + 8), 18, BALATRO_WHITE);
...
static void draw_phase_shop(const State *state, const LegalMasks *legal, Rectangle rect, Action *out_action, Vector2 mouse, Client *client) {
    ...
    bool can_reroll = can_afford(state, state->reroll_cost);
    ...
    const char *reroll_text = TextFormat("REROLL  $%d", state->reroll_cost);
    ...
    for (int i = 0; i < state->shop_main_count; ++i) {
        ...
        if (set == SET_JOKER) {
            draw_joker_card(sc, card_r, hovered, i, true, sc->cost, client->puffer);
        } else if (set == SET_TAROT || set == SET_PLANET || set == SET_SPECTRAL) {
            draw_consumable_card(sc, card_r, hovered, i, true, sc->cost);
        } else {
            draw_playing_card(sc, card_r, false, hovered, true, sc->cost);
        }
    ...
    for (int i = 0; i < state->shop_booster_count; ++i) {
        ...
        if (kind == 1) {
            ...
            DrawTriangle((Vector2){motif.x, motif.y - 7}, (Vector2){motif.x - 23, motif.y - 29},
                         (Vector2){motif.x - 7, motif.y + 2}, BALATRO_RED_MULT);
            DrawTriangle((Vector2){motif.x, motif.y - 7}, (Vector2){motif.x + 7, motif.y + 2},
                         (Vector2){motif.x + 23, motif.y - 29}, BALATRO_GOLD);
            DrawCircleV((Vector2){motif.x - 23, motif.y - 31}, 4, BALATRO_GOLD);
            DrawCircleV((Vector2){motif.x + 23, motif.y - 31}, 4, BALATRO_RED_MULT);
            DrawCircleV((Vector2){motif.x, motif.y + 8}, 16, BALATRO_CARD_WHITE);
            DrawCircleV((Vector2){motif.x - 6, motif.y + 5}, 2.5f, BALATRO_RED_MULT);
            DrawCircleV((Vector2){motif.x + 6, motif.y + 5}, 2.5f, BALATRO_BLUE_CHIPS);
            DrawLineEx((Vector2){motif.x - 6, motif.y + 15}, (Vector2){motif.x + 6, motif.y + 15}, 2, BALATRO_DARK_TEXT);
        }
...
static void draw_phase_pack_opening(const State *state, const LegalMasks *legal, Rectangle rect, int selected_count, const uint8_t *selected_indices, Action *out_action, Vector2 mouse, Client *client) {
    ...
    for (int i = 0; i < state->pack_count; ++i) {
        ...
        if (set == SET_JOKER) {
            draw_joker_card(pc, c_rect, hovered, i, false, 0, client->puffer);
        } else if (set == SET_TAROT || set == SET_PLANET || set == SET_SPECTRAL) {
            draw_consumable_card(pc, c_rect, hovered, i, false, 0);
        } else {
            draw_playing_card(pc, c_rect, false, hovered, false, 0);
        }
...
static void draw_tooltip(const Client *client) {
    ...
    DrawRectangleRounded(tr, 0.12f, 3, (Color){15, 20, 26, 250});
    DrawRectangleRoundedLinesEx(tr, 0.12f, 3, 1.5f, BALATRO_GOLD);

    DrawText(client->tooltip_title, (int)(tx + 10), (int)(ty + 8), 13, BALATRO_GOLD);
    DrawText(client->tooltip_type, (int)(tx + 10), (int)(ty + 25), 10, (Color){170, 195, 220, 255});
    draw_multiline_text(client->tooltip_desc, (int)(tx + 10), (int)(ty + 40), (int)(tw - 20), 10, BALATRO_WHITE);
    DrawText(client->tooltip_stats, (int)(tx + 10), (int)(ty + th - 16), 10, (Color){140, 230, 160, 255});
}
...
static inline Client *make_client(int width, int height) {
    ...
    InitWindow(client->width, client->height, "PufferLib Balatro Showcase (RayLib)");
    SetTargetFPS(60);
...
static inline void balatro_render(Env *env) {
    ...
    if (env->state.phase == PHASE_SELECTING_HAND) {
    ...
    if (action.type != UINT8_MAX) {
        ...
        StepResult res = {0};
        apply_step(&env->state, &action, &env->legal_masks, &res);
        if (action.type == ACTION_PLAY_HAND) {
