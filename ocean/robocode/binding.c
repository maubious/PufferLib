...
void my_init(Env* env, Dict* kwargs) {
    ...
    DictItem* block_item = dict_get_unsafe(kwargs, "action_block_p");
    env->action_block_p = block_item ? (float)block_item->value : 0.0f;
...
void my_log(Log* log, Dict* out) {
    ...
    dict_set(out, "action_block_rate", log->action_block_rate);
